	...
	while True:
		...
		try:
			...
			if addr != host:
				...
				if status != 'processing' and sta.decode() == 'passing' and 'processing' not in neighborhood.values(): # ha valaki kuld egy 'passing'-ot es nem 'processing' akkor hepp
					status = 'hepp'
...
def compute_token():
	...
	if 'processing' in neighborhood.values():
		...
		return True
	try:
		if ( measure.distance < 15 or measure.intruder > 3 ) and not table:
			print( 'hepp' )
			play_hepp( hepps[ random.randint( 1, 49 ) ] )
		elif ( measure.distance < 15 or measure.intruder > 3 ) and table:
			print( 'table goes out' )
			play_hepp( tables[ table ][ 'tableGO' ], porond )
			table = False
		elif ( measure.distance > 15 or measure.intruder < 3 ) and not table:
			print( 'table come in' )
